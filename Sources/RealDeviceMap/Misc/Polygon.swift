...
extension Polygon {
    ...
    func getNonInternalS2CellIDs(minLevel: Int, maxLevel: Int, maxCells: Int) -> [S2CellId] {

        let bbox = BoundingBox(from: coordinates[0])
        let region = S2LatLngRect(
            lo: S2LatLng(
                lat: S1Angle(degrees: bbox!.southEast.latitude - 0.01),
                lng: S1Angle(degrees: bbox!.northWest.longitude - 0.02)
            ),
            hi: S2LatLng(
                lat: S1Angle(degrees: bbox!.northWest.latitude + 0.01),
                lng: S1Angle(degrees: bbox!.southEast.longitude + 0.02)
            )
        )
        let regionCoverer = S2RegionCoverer()
        regionCoverer.maxCells = maxCells
